...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // Thread* thisThread = pos.this_thread();//From MCTS scores
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
              ...
              || cutNode))
      {
          ...
          int threshold = 4500;
          if (percSearch && thisThread->perceptronAccuracy > threshold)
            r -= prediction * (thisThread->perceptronAccuracy - threshold) / 100;
